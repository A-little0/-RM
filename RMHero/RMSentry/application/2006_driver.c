...
//	for(int i=0;i< M2006_RECORD_EXP_ECDBOSX_LENGTH;i++){device->record_exp_ecdboxs[i]=0;}
	...
	if(pidmode==PID_POSITION)
	{
		switch(0x200+motorid)
		{
			...
			pid_speed[0]=DOWN2006_SPEEDPID_POSITION_KP;
			pid_speed[1]=DOWN2006_SPEEDPID_POSITION_KI;
			pid_speed[2]=DOWN2006_SPEEDPID_POSITION_KD;
			
				break;
			case  CAN_TRIGGER_UP_MOTOR_ID:
			
			pid_speed[0]=UP2006_SPEEDPID_POSITION_KP;
			pid_speed[1]=UP2006_SPEEDPID_POSITION_KI;
			pid_speed[2]=UP2006_SPEEDPID_POSITION_KD;
	...
	else if(pidmode == PID_DELTA)
	{
		switch(0x200+motorid)
		{
			case CAN_TRIGGER_DOMN_MOTOR_ID:		
			
			pid_speed[0]=DOWN2006_SPEEDPID_DELTA_KP;
			pid_speed[1]=DOWN2006_SPEEDPID_DELTA_KI;
			pid_speed[2]=DOWN2006_SPEEDPID_DELTA_KD;
