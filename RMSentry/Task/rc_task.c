...
void KeyBoard_Process(int mode)
{
    switch(mode)
	{
		...
		case CHASSIS_GYRO_MODE:{
			...
			/*发射控制*/
			if(RC_Ctl.mouse.press_l == 1){SetLeft_Gun(LEFT_SHOT_SPEED);}
			else{SetLeft_Gun(LEFT_STOP_SPEED);}
			if(RC_Ctl.mouse.press_r == 1){SetRight_Gun(RIGHT_SHOT_SPEED);}
			else{SetRight_Gun(RIGHT_STOP_SPEED);}
...
void RC_Process(void)
{
	...
	if((RC_Ctl.key.v & KEY_PRESSED_OFFSET_Q )!= 0)
	{
	   if((RC_Ctl.key.last_v & KEY_PRESSED_OFFSET_Q)!=(RC_Ctl.key.v & KEY_PRESSED_OFFSET_Q ))
	   {
			key_mode++;
	   }
	   if(key_mode == 1)
	   {
		  keyboard_user.s1=CHASSIS_FOLLOW_GIMBAL_MODE;
	   }
	   else if(key_mode == 2)
	   {
		  keyboard_user.s1=CHASSIS_GYRO_MODE;
	   }
	   else
	   {
			key_mode =1;
...
//	else if((RC_Ctl.key.v & KEY_PRESSED_OFFSET_E )!= 0 )
//	{
//		keyboard_user.s1=CHASSIS_GYRO_MODE;
//	}
